...
from retrieval.models import IntentResult, SessionMessage, Stage2AResult, Stage2BResult
...
class Stage1Clarifier:
    ...
    def clarify(self, query: str, history: List[SessionMessage]) -> str:
        ...
        hist_text = "\n".join(
            ...
            for i, m in enumerate(history[-2:])  # last 2 turns sufficient for dependency resolution
        )
...
# Taxo.online citation patterns — all variants normalised to canonical form
_CITATION_PATTERNS = [
    # "2017 taxo.online 42" / "2017 taxo online 42" / "17 taxo online 42"
    re.compile(r"(\d{2,4})\s+taxo[\s.]?online\s+(\d+)", re.IGNORECASE),
    # "taxo online 42" / "taxoonline 42" / "taxo 42"
    re.compile(r"taxo[\s.]?(?:online\s+)?(\d+)(?!\d)", re.IGNORECASE),
    # "MANU/SC/0872/2021"
    re.compile(r"(MANU/[A-Z]+/\d+/\d+)", re.IGNORECASE),
    # "2017 SCC Online 234"
    re.compile(r"(\d{4})\s+SCC\s+Online\s+(\d+)", re.IGNORECASE),
]
...
def _extract_citation(text: str) -> Optional[str]:
    ...
    # taxo online / taxoonline / taxo (number only)
...
3. SCORE_WEIGHTS: additive score boosts per chunk_type.
   ...
   Examples:
   "what is section 129 of CGST Act" → GENERAL:
     {"cgst_section": 0.10, "igst_section": 0.05, "cgst_rule": 0.05,
      "notification": 0.05, "circular": 0.05, "judgment": 0.05}

   "give me judgement of section 17" → JUDGMENT:
     {"judgment": 0.10, "cgst_section": 0.05, "igst_section": 0.05,
      "notification": 0.05, "circular": 0.05}

   "GST rate on gym membership" → RATE:
     {"sac_code": 0.10, "notification": 0.10, "circular": 0.05}

   "show me FORM GST REG-30" → FORM:
     {"gst_form": 0.10, "cgst_rule": 0.05, "notification": 0.05}
...
class Stage2CIntent:
    ...
    def classify(self, query: str) -> IntentResult:
        ...
        try:
            ...
            # Enforce analytical_review second-to-last, summary last
...
# Common tax abbreviations and their expanded forms.
# Both the abbreviation AND the expansion are kept in the keyword document
# so BM25 matches whichever form was used at indexing time.
# Weight 3 (same as L1 tokens) — expansion is high-confidence signal.

_BM25_EXPANSIONS: Dict[str, List[str]] = {
    # These are 100% unambiguous single-meaning abbreviations in Indian tax law.
    # Both the abbreviation AND the expansion are added so BM25 matches
    # whichever form was used at indexing time.
...
def _expand_tokens(tokens: List[str]) -> List[str]:
    """
    For each token that is a known abbreviation, add its expansions.
    Both the original token AND its expansions are kept.
    """
...
def build_bm25_keyword_document(a: Stage2AResult, b: Stage2BResult) -> str:
    ...
    # Query expansion — add expanded forms for known abbreviations.
    # Both original token and expansion kept (expand_tokens preserves originals).
    # Applied at weight 3 same as L1 — high-confidence signal.
    l1 = _expand_tokens(l1)
    # Also expand L3 regex tokens (weight 1 path)
    parts = _expand_tokens(parts)

    parts += l1 * 3
    doc = " ".join(t for t in parts if t)
    original_count = len(doc.split()) - len([t for t in doc.split() if any(t in exps for exps in _BM25_EXPANSIONS.values())])
...
def _format_history(messages: List[SessionMessage]) -> str:
    lines = []
    for i, m in enumerate(messages, 1):
        lines.append(f"[{i}] User: {m.user_query}")
        lines.append(f"    Assistant: {m.llm_response[:300]}")
    return "\n".join(lines)