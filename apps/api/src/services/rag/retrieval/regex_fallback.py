...
Only extracts these 4 fields â€” everything else requires LLM.
"""
...
# â”€â”€ Compiled patterns â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Token builders â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _is_valid_subclause(s: str) -> bool:
    ...
    Valid: (a), (1), (1A), (ba), (xiii), (2b) â€” short, no spaces, starts with digit/letter
    Invalid: (pg.S), (pg.II), (pg.ll), (as a supplier of...), (construction of...)
    """
    if not s or len(s) > 8 or ' ' in s:
        return False
    # Must start with digit or letter â€” not "pg", punctuation etc.
...
def _section_from_groups(groups) -> str | None:
    ...
    if sec_plain:
        # Extract ONLY number + max 2-letter suffix â€” stops at title text
        # "9compositionlevy" â†’ "9"  |  "14A" â†’ "14a"  |  "11AC" â†’ "11ac"
        import re as _re
        ...
        if sub_plain:
            ...
            # Valid sub-clauses: short (â‰¤8), no spaces, start with digit/letter
...
def _rule_from_match(m: re.Match) -> str | None:
    ...
    if rule_of:
        # subrule (2) of Rule 90 â†’ rule_90_2
        num_str = rule_of
        sub_str = sub_of or ""
    else:
    ...
    # Rule 89(2) â†’ rule_89_2  (underscore separator â€” not rule_892)
    if sub_str:
        sub_stripped = sub_str.strip()
        if _is_valid_subclause(sub_stripped):
            return f"rule_{_clean(num_str)}_{_clean(sub_stripped)}"
        # Title or page ref in brackets â€” return just the rule number
...
# â”€â”€ Public extractor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def extract_fallback(text: str) -> Dict[str, List[str]]:
    ...
    Two things happen:
      1. Entire raw text is whitespace-split â€” every token added to BM25.
    ...
    # â”€â”€ Step 1: Whitespace split entire text â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Clean and split â€” everything goes into BM25 as raw tokens
    ...
    for t in raw_tokens_raw:
        ...
        t = t.strip('.,;:()"\'\-_[]{}!?')
        # Remove trailing/leading brackets that got glued to words: "2(appeal" â†’ "2"
    ...
    # â”€â”€ Step 2: Regex extract normalised legal refs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    sections      = []
    rules         = []
    notifications = []
    circulars     = []
    ...
    # (normalised form replaces raw form â€” no duplication)
    ...
    # â”€â”€ Step 3: Build final raw token list â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # Raw whitespace tokens go into topics â€” merger handles them as L3 tokens
    # weight 1x â€” lower than L1 metadata but still indexed
