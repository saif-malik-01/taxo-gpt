...
@dataclass
class QdrantConfig:
    target_host:         str           = os.getenv("QDRANT_HOST", "localhost")
    ...
    text_vector_name:    str           = os.getenv("QDRANT_TEXT_VECTOR", "text_vector")
    summary_vector_name: str           = os.getenv("QDRANT_SUMMARY_VECTOR", "summary_vector")
    sparse_vector_name:  str           = os.getenv("QDRANT_SPARSE_VECTOR", "bm25_sparse")
    vector_size:         int           = 1024
    timeout:             int           = 60

    @property
    def host(self) -> str:
        # Standardize host: remove http/https prefix if accidentally included
        h = self.target_host.strip()
        if "://" in h:
            h = h.split("://")[-1]
        return h
