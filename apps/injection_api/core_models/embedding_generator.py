...
class TitanEmbeddingGenerator:
    ...
    def _invoke(self, text: str) -> Optional[List[float]]:
        ...
        body = {
            "inputText":  text[:8000],
            "dimensions": CONFIG.bedrock.titan_dimensions,
            "normalize":  CONFIG.bedrock.titan_normalize,
        }

        for attempt in range(1, CONFIG.pipeline.max_retries + 1):
            try:
                response = self._client.invoke_model(
                    modelId     = CONFIG.bedrock.titan_model_id,
                ...
                if embedding and len(embedding) == CONFIG.bedrock.titan_dimensions:
                    return embedding
                ...
                if code in ("ThrottlingException", "ServiceUnavailableException"):
                    time.sleep(CONFIG.pipeline.retry_delay_seconds * attempt * 2)
                elif code == "ValidationException":
                    logger.error(f"Titan ValidationException: {e.response['Error']['Message']}")
                    return None
                elif attempt == CONFIG.pipeline.max_retries:
                    return None
                else:
                    time.sleep(CONFIG.pipeline.retry_delay_seconds)
