...
from flow.config import PRESERVED_UPPERCASE, AUTHORITY_MAP
...
def _extract_ref_number(stripped: str) -> str:
    """
    Extract ONLY the leading number+bracket part, ignoring trailing title text.

    "7 - Inter-State Supply"   → "7"       (title after dash discarded)
    "8 Tax Liability on..."    → "8"       (title after space discarded)
    "2 pg.II"                  → "2"       (page ref discarded)
    "14A"                      → "14a"     (valid letter suffix kept)
    "14(a)"                    → "14_a"    (sub-bracket kept)
    "80C(2)(xiii)"             → "80c_2_xiii"
    "89(2)(b)"                 → "89_2_b"
    """
...
def normalise_section_list(items: List[str]) -> List[str]:
    ...
    for item in items:
        ...
        for part in _split_compound(str(item)):
            ...
            # Handle "sub-section (1A) of section 122" -> section_122_1a
            sub_m = re.match(
            ...
            # Handle "Section 73/74" -> section_73 + section_74
            slash_m = re.match(
...
def normalise_rule_list(items: List[str]) -> List[str]:
    ...
    for item in items:
        ...
        for part in _split_compound(str(item)):
            ...
            # Handle "subrule (X) of Rule Y" → rule_Y_X
            sub_m = re.match(
...
def normalise_gazette(value: str) -> Optional[str]:
    """G.S.R. (E) → gsr_e  |  G.S.R. 818(E) → gsr_818e"""
    if not value:
        return None
    v = value.lower()
